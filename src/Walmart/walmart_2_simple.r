...
###############################################
## Test Data load and clean.  Build a model that just averages the 
## Take just the Date and the Store Id and Catogory Id.
## Fuzzy the Date and do a group by
## Remove Holiday
###############################################
...
samples_taken = 421570/100
ind <- sample(length(train[,1]),samples_taken,FALSE)
train_df <- tbl_df(train[ind,1:4])
#train_df <- tbl_df(train[,1:4]) #All data
feature_df <- tbl_df(feature)
training <- inner_join(train_df, feature_df, by=c('Cat_Store','Date'))
training <- training[, c(4,1,3,2)] 
XtrainClean = cleanInputDataForGBM(training)
XtrainClean = XtrainClean[, c(1,2,4,5,6)]  

#Now Group by the Cat_Store, Quant_Year, Quant_Month
XtrainClean = XtrainClean %.% group_by(Cat_Dept, Quant_Year, Quant_Month) %.% summarise(average_weekly_sales = mean(Qaunt_Weekly_Sales)) 
XtrainClean = XtrainClean[, c(5,1:4)]
...
XtestClean = XtestClean[, c(2,4,5)]   


#Now Group by the Cat_Store, Quant_Year, Quant_Month
#XtestClean= XtestClean %.% group_by(Cat_Store, Cat_Dept, Quant_Year, Quant_Month) %.% summarise(average_weekly_sales = mean(Qaunt_Weekly_Sales)) 
