...
# Rev4 - Improve Married Men data and code the tickets 0.83 ( ) => [0.95,0.408]
# Use 3000 trees, 0.01 error, and depth = 3, 0.78 ( ) => [0.977,0.46]
require(gbm)
library(ggplot2)
library(calibrate)
library(grid)
...
#trainCols = c(3,4) # gender and age
#trainCols = c(3,4) # gender and age
...
# Remove unused categorical values as NA in test set
id <- which(!(XtestClean$Cat_TicketName %in% levels(XtrainClean$Cat_TicketName)))
XtestCleanFact <- XtestClean
levels_to_delete = XtestCleanFact$Cat_TicketName[id]
XtestCleanFact$Cat_TicketName[id] <- NA
XtestCleanFact$Cat_TicketName<- factor(XtestCleanFact$Cat_TicketName)
Yhattest <- predict.gbm(mo1gbm, newdata=XtestCleanFact[trainCols], n.trees = ntrees, type="response")
XtestClean = XtestCleanFact
...
write.csv(YhattrainBool, "titanic_4_gbm_train.csv", row.names=FALSE)
...
write.table(as.matrix(YhattestBool)[,1], "titanic_4_gbm_test.csv", row.names=FALSE, col.names=FALSE)
