...
# Rev6 - GBM use depth=20 and 500 trees 0.84 ( ) => [0.94, 0.37]
# Use GBM + RF together
...
ret = predictGBM(Y, XtrainClean, XtestClean, trainCols)
Yhattrain = ret[[1]]
Yhattest = ret[[2]]
...
precentCorr
write.csv(YhattrainBool, "titanic_6_gbm_train.csv", row.names=FALSE)
...
write.table(as.matrix(YhattestBool)[,1], "titanic_6_gbm_test.csv", row.names=FALSE, col.names=FALSE)
